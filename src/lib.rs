  ...
  fn draw(&mut self, context: &mut Context) -> GameResult<()> {
    ...
    for width in 0..self.cell_count_row {
      graphics::draw(
        ...
        graphics::DrawParam::default().dest(Point2::new(width as f32 * self.cell_width, 0.0)),
      )?;
    ...
    graphics::draw(
      context,
      &self.grid_line_vertical,
      graphics::DrawParam::default().dest(Point2::new(arena_width - 1.0, 0.0)),
    )?;
    graphics::draw(
      context,
      &self.grid_line_vertical,
      graphics::DrawParam::default()
        .dest(Point2::new(0.0, arena_height))
        .rotation(-std::f32::consts::FRAC_PI_2),
    )?;
